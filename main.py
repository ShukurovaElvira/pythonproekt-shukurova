...
def draw_point(x, y, type):
        ...
        if type == 0:
            color = "yellow"
            id = canvas.create_oval(x * step_x, y * step_y, x * step_x + step_x, y * step_y + step_y, fill="yellow")
            id2 = canvas.create_oval(x * step_x + size, y * step_y + size, x * step_x + step_y - size, y * step_y + step_y - size, fill="white")
            list_ids.append(id)
            list_ids.append(id2)
        if type == 1:
            color = "blue"
...
b1 = Button(tk, text="Очистить поле!", command=button_press)
b1.pack()
